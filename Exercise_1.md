<p align="center">
<font size = 8> Описание контейнерного инструмента Grafana </font>
</p>
...
Для лучшего управления ресурсами и исключени конфликтов и другим ошибок рекомендуется создать новое пространство имен (namespace) в Kubernetes

1. Что бы сздать ``namespace``, выполните следующую команду:
   ~~~ Bash
...
2. Что бы проверить и посмотреть созданный ``namespace``, выполните следующую команду:
   ~~~ Bash
   kubectl get namespace my-grafana
   ~~~

3. Создайте файл YAML манифест ``grafana.yaml``. Этот файл будет содержать необходимый код для развертывания.
   ~~~ Bash
   touch grafana.yaml
   ~~~
...
### Обновите существующее развертывание используя стратегию непрерывного обновления

Непрырывные обновления позволяют выполнять обновления развертывания без простоев за счет постепенного обновления модулей (Pods) новыми.
...
3. Чтобы просмотреть историю развертывания и проверить изменения, выполните следующую команду:
   ...
   kubectl rollout history deployment/grafana --namespace=my-grafana
   ~~~
   Вы должны увидеть обновленную информацию, которую вы добавили в ``CHANGE-CAUSE`` ранее.
...
4. Что бы убедиться, что развертывание в кластере прошло успешно, выполните следующую команду:
   ~~~ Bash
...
5. Чтобы проверить состояния всех развернутых объектов, выполните следующую команду и включите ключ (дополнительная приписка к стандартной команде) ``-o wide`` для получения более подробных данных:
   ~~~ Bash
   kubectl get all --namespace=my-grafana -o wide
   ~~~
   Вы должны увидеть недавно развернутый образ ``grafana-oss-dev``.
...
10. Для проверки выполните команду ``kubectl rollout history``:
   ...
   Последняя строка вывода команды ``kubectl rollout history deployment`` - это версия, которая в данный момент активна и выполняется в вашей среде Kubernetes.

### Откат развертывания
Когда развертывание Grafana становится нестабильным из-за аварийного цикла, ошибок и т.д., вы можете откатить развертывание до более ранней версии (a ``REVISION``).

По умолчанию история развертывания Kubernetes сохраняется в системе, так что вы можете выполнить откат в любое время.
...
7. Чтобы просмотреть историю новых развертываний, выполните следующую команду:
   ...
   kubectl rollout history deployment grafana --namespace=my-grafana
   ~~~
   Если вам нужно вернуться к любому другому варианту ``REVISION``, просто повторите описанные выше шаги и используйте правильный номер редакции в параметре ``--to-revision``.


### Удалиение Grafana

Если вы хотите удалить какой-либо из объектов развертывания Grafana, используйте ``kubectl delete command``.

1. Если вы хотите удалить полное развертывание Grafana, выполните следующую команду:
   ~~~ Bash
